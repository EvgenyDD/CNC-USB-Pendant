...
uint8_t usbd_dfu_setup(void *pdev, USBD_SetupReqTypedef *req)
{
...
#endif
				// fw_header_check_all();
				fw_sts[0] = 100; // fw_sts[0] = g_fw_info[0].locked;
				fw_sts[1] = 100; // fw_sts[1] = g_fw_info[1].locked;
				fw_sts[2] = 100; // fw_sts[2] = g_fw_info[2].locked;
				   ...
				   upload.size < USBD_BUF_SZ)
				{
					...
					else
					{
...
#endif // DFU_READS_CFG_SECTION
						{
							...
							{
								USBD_CtlSendData(pdev, /*(uint8_t *)g_fw_info[req->wValue].addr*/NULL, 0);
							}
							// else
							// {
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_GET_DESCRIPTOR:
		{
			...
			{
				pbuf = usbd_dfu_cfg_desc + 9 + (9 * USBD_MAX_NUM_INTERFACES);
	...
	if(dl_pending)
	{
		...
		else
		{
			...
			if(fw_index_sel == FW_APP + 1)
			{
				...
				sts = 1; // sts = platform_flash_write(CFG_ORIGIN + addr_off, &usbd_buffer[4], size_to_write);
			}
			else
#endif
			{
				...
				sts = 1; // sts = platform_flash_write(ADDR_ORIGIN + addr_off, &usbd_buffer[4], size_to_write);
			}
			dwnload_was = true;
			dl_pending = false;
